/**
 * LoginPage — email + password sign-in. Persists the session via AuthContext,
 * then navigates to the page the user originally requested (or '/').
 */
...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setErr(extractErr(e) ?? t('auth.invalidCredentials'));
    } finally {
      setBusy(false);
    }
  }
...
function extractErr(e: unknown): string | null {
  ...
  if (typeof e === 'object' && e !== null) {
    ...
    return ax.response?.data?.detail ?? ax.response?.data?.message ?? (e as Error).message ?? null;
  }
  return String(e);
}
